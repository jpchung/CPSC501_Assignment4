 ...
 * Algorithm-Based Optimization of Baseline program (CODE TUNING OPTIMIZATION 5):
...
int main(int argc, char* argv[]){
    ...
    if(argc <= 3){
        ...
        if(fileExtFlag == 1){
            ...
            //OPTIMIZATION 1 - CODE JAMMING
            //OPTIMIZATION 3 - UNROLLING (PARTIAL)
            for(int i  = 0; i < maxLengthPow2; i+=3){
               freqX[i] = 0.0;
               freqX[i+1] = 0.0;
               freqX[i+2] = 0.0;
...
void convolveFreqs(double* freqX, double *freqH, double* freqY, int arrayLength){
    ...
    for(int i = 0; i < arrayLength; i+= 2){
        ...
        //OPTIMIZATION 5 - MINIMIZE ARRAY REFERENCES
        double realX = freqX[i];
        double imX = freqX[i+1];
        double realH = freqH[i];
        double imH = freqH[i+1];
        freqY[i] = (realX * realH) - (imX * imH); //real
        freqY[i+1] = (imX * realH) + (realX * imH); //imaginary
    
        //OPTIMIZATION 2 - MINIMIZING WORK INSIDE LOOPS
        //if((i%100000) == 0)
        //    printf("Convolving %d...\n", i);
    }
}
...
void scaleOutputFreq(double* outFreq, WavFile* inputFile, int numSamples){
    ...
    //OPTIMIZATION 2 - MINIMIZING WORK INSIDE LOOPS
    double maxValueRatio = inputMaxValue/outputMaxValue;
    for(int i  = 0; i < numSamples; i++){
        //outFreq[i] = outFreq[i] / outputMaxValue * inputMaxValue;
        outFreq[i] = outFreq[i] * maxValueRatio;
    }
